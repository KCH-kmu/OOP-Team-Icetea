...
void MainMenu::Show()
{
    ...
    int logoActualWidth = logoWidth;
    logoWidth = 80;
    int logoLeftPad = (logoActualWidth < logoWidth) ? (logoWidth - logoActualWidth + 1) / 2 - 3 : 0;
    ...
    do
    {
        ...
        while ((logoNext = projectLogo.find('\n', logoPos)) != string::npos)
        {
            string logoLine = projectLogo.substr(logoPos, logoNext - logoPos);
            for (int i = 0; i < logoLeftPad; i++) cout << " ";
            cout << logoLine << "\n";
            logoPos = logoNext + 1;
        }
        cout << endl;

        PrintCentered("Team 아이스티 문제은행 프로그램", logoWidth);
        cout << endl;
